...
exports.verifyAllLaws = verifyAllLaws;
exports.enforceLaws = enforceLaws;
const fast_check_1 = __importDefault(require("fast-check"));
// Law verification functions
function verifyAllLaws(laws) {
    const results = [];
...
function verifyPurity(laws) {
    try {
        fast_check_1.default.assert(fast_check_1.default.property(laws.generateEvents(), (events) => {
...
function verifyFunctoriality(laws) {
    try {
        // R(id) = id and R(g∘f) = R(g)∘R(f)
        // At Spec level, this is just function composition
        const events = []; // Empty events array for identity test
        const composed = (s) => laws.fold(laws.evolve, s, events);
        const identity = (s) => s;
...
function verifyObservabilityNaturality(laws) {
    try {
        // O(g∘f) = O(g)∘O(f) - observability composes correctly
        const mockMetrics = (name, f) => f; // Simplified
        return {
...
function verifyCQRSCommutativity(laws) {
    try {
        fast_check_1.default.assert(fast_check_1.default.property(laws.generateEvents(), (events) => {
...
function verifyOutboxCommutativity(laws) {
    try {
        ...
        let passed = true;
        verifyOutboxLaw(infrastructure).then((result) => passed = result).catch(() => passed = false);
        return {
            law: '5. Outbox Commutativity',
...
function verifyPushPullEquivalence(laws) {
    try {
...
function verifyReplayDeterminism(laws) {
    try {
        fast_check_1.default.assert(fast_check_1.default.property(laws.generateEvents(), laws.generateEvents(), (events1, events2) => {
...
function verifyIdempotence(laws) {
    try {
...
function verifyCausalityOrdering(laws) {
    try {
...
function verifyMonoidalAggregation(laws) {
    return {
        law: '10. Monoidal Aggregation',
...
function verifyPullbackCorrectness(laws) {
    return {
        law: '11. Pullback Correctness',
...
function verifyPushoutSchemaEvolution(laws) {
    return {
        law: '12. Pushout Schema Evolution',
