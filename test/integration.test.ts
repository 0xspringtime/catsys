...
import { composeForTesting } from '../src/app/compose';
import { s0, type Command, type Raw } from '../src/spec';

describe('URL Shortener Integration', () => {
  test('Complete flow: normalize → validate → decide → persist → publish → fold', async () => {
    const app = composeForTesting();
    let state = s0;
  ...
  test('Health check and system status', async () => {
    ...
    expect(state).toEqual(s0); // Initial state
  });
  ...
  test('Error handling and validation', async () => {
    ...
    const { result } = await app.httpHandle(s0, invalidRaw);
    ...
    await expect(app.handle(s0, invalidCommand)).rejects.toThrow();
  });
  ...
  test('Demonstrates all 12 laws in practice', async () => {
    ...
    const command: Command = { kind: 'Create', long: 'https://test.com', id: 'test1' };
    ...
    const cmd1: Command = { kind: 'Create', long: 'https://test1.com', id: 'c1' };
    const cmd2: Command = { kind: 'Resolve', short: 'test123', id: 'c2' };
